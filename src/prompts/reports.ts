...
- 中段: ミニスパークライン（横長・小）
- 下段: 地合い（やさしい補足つき）＋（任意）主要ライン 2 本を small text
...
3. 地合い（初見でも分かるように）— analyze_market_signal を「強気 / 弱気 / 方向感なし」で示し、専門語はかみ砕く（補足は 2〜3 行・数値テーブルは出さない）:
   - スコアは「方向感スコア」と呼び、必ずスケールを添える（-100〜+100、＋＝上向き / −＝下向き、おおむね ±25 超で強気・弱気、中間は方向感なし）。
   - スコアは単一指標ではなく複数サインの総合だと明示する（移動平均の向き＝最重視 / 値動きの勢い / 売買の流れ＝買い・売りの偏り / 値動きの荒さ / 買い圧力）。表示する「主因」は今回特に効いた上位要因にすぎない旨も一言添える。
   - 「信頼度」は「読みの確からしさ（高 / 中 / 低）」と言い換える。指標名も日常語に（SMAトレンド→移動平均の向き、モメンタム→値動きの勢い、CVD→買い/売りの勢い、ボラティリティ→値動きの荒さ）。
