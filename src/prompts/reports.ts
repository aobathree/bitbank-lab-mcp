...
3. 地合い（初見でも分かるように）— analyze_market_signal を「強気 / 弱気 / 方向感なし」で示し、専門語はかみ砕く（補足は 1〜2 行・数値テーブルは出さない）:
   - スコアは「方向感スコア」と呼び、必ずスケールを添える（-100〜+100、＋＝上向き / −＝下向き、おおむね ±25 超で強気・弱気、中間は方向感なし）。
   - 「信頼度」は「読みの確からしさ（高 / 中 / 低）」と言い換える。
   - 主因の指標名も日常語に: SMAトレンド→移動平均の向き、モメンタム→値動きの勢い、CVD→買い/売りの勢い、ボラティリティ→値動きの荒さ。
